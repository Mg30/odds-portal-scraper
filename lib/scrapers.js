...
async function extractOddsData(page, callback, leagueName) {
    ...
    await page.waitForSelector('div[data-testid="game-row"]')
    const links = await page.$$eval('div[data-testid="game-row"] > a', els => els.map(el => el.getAttribute("href")));
    ...
    for (const link of uniqueLinks) {
        try {
            ...
            const mlFirstHalf = await extractMlOdds(page, "1stHalf");
            const mlSecondHalf = await extractMlOdds(page, "2ndHalf");
...
async function extractMlOdds(page, oddTypeChoosen) {
    ...
    const buttonSelector = `div[data-testid="kickoff-events-nav"] > div`
    await page.waitForSelector(buttonSelector)
    await page.$$eval(buttonSelector, (els, oddType) => {
        return els.find(el => el.innerText.replaceAll("\n", "").replaceAll(" ", "") === oddType).click()
    }, oddTypeChoosen);
    await page.waitForTimeout(2000)
...
async function extractOverUnderOdds(page, underOver) {
    ...
    logger.info(`scrapping odd for under/over ${underOver}`)
    ...
    const data = await page.evaluate(() => {
        const data = [];
        const rows = document.querySelectorAll('div[data-v-59b97132].border-black-borders');
        rows.forEach(row => {
