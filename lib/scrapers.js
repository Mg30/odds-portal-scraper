...
async function extractMlOdds(page, oddTypeChoosen) {
    ...
    const buttons = await page.$$eval(buttonSelector, (els, oddType) => {
        return els.map(el => el.innerText.replaceAll("\n", "").replaceAll(" ", "") === oddType)
    }, oddTypeChoosen);
...
async function historicScraper(browser, leagueName, startYear, endYear, oddsFormat, callback) {
    ...
    for (const year of years) {
        if (['mls', 'brazil-serie-a', 'premier-league', 'eredivisie'].includes(leagueName)) {
