...
async function extractOddsData(page) {
    ...
    let data = await page.$$eval('div[set]', els => els.map(el => {
        ...
        const teams = Array.from(el.querySelectorAll('a[title]')).map(el => el.getAttribute("title"));
...
async function scrapeOddsHistoric(page, oddsFormat) {
    ...
    let pages = await page.$$eval("div.pagination > a", (els) => (els.map(el => el.textContent)));
    ...
    for (const pag of pages) {
        if (pag != 'Next') {
            ...
            await page.$eval(`div.pagination > a[data-number="${intPag}"]`, el => el.click());
