...
async function extractMlOdds(page, id) {
    ...
    await page.waitForSelector('div.h-9.border-b[data-v-4905fa49]')
    return await page.$$eval('div.h-9.border-b[data-v-4905fa49]', els => els.map(el => {
        const bookMakerName = el.querySelector('div[providername] > a[target] > p').textContent;
        const odds = Array.from(el.querySelectorAll('div[data-v-4905fa49] > p[data-v-4905fa49]')).map(elements => elements.textContent);
        const [hw, d, aw] = odds
        return {
            bookMakerName,
            hw,
