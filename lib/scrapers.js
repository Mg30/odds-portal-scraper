...
async function extractOddsData(page, callback, leagueName) {
    ...
    const links = await page.$$eval('a[title][data-v-74bb5442]', els => els.map(el => el.getAttribute("href")));
...
async function extractMlOdds(page, id) {
    ...
    await page.waitForSelector('div.h-9[data-v-11c8c78a]')
    return await page.$$eval('div.h-9[data-v-11c8c78a]', els => els.map(el => {
        const bookMakerName = el.querySelector('img.bookmaker-logo').getAttribute("title");
        const odds = Array.from(el.querySelectorAll('div[data-v-11c8c78a] > p[data-v-11c8c78a]')).map(elements => elements.textContent);
        const [hw, d, aw] = odds
        return {
            bookMakerName,
            hw,
